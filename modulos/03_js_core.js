...
function _applyZoom(){
  ...
  /* Re-aplicar rotación y centrar cuando hay rotación activa */
  if(_planRot!==0){_applyPlanRotation();}else{setTimeout(_centerView,0);}
}
...
/* Centra el plan en el viewport del zoom-wrap (funciona con cualquier ángulo y zoom) */
function _centerView(){
  var ml=document.getElementById('markerLayer');
  var wrap=document.getElementById('zoom-wrap');
  var mt=parseFloat(ml.style.marginTop)||0;
  /* Centro del contenido visual = siempre el centro del markerLayer */
  var cx=ml.offsetWidth/2;
  var cy=mt+ml.offsetHeight/2;
  /* Scroll para que ese centro quede en el centro del viewport */
